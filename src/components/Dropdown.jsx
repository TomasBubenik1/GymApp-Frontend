...
export default function DropdownMenuFilter({
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <button className="mt-6 w-full text-white bg-foreground relative rounded-md p-2 justify-center flex flex-col items-center">
          {options.map((option, i) => {
            if (selectedOption === option) {
              return (
                <p className="font-semibold text-accent" onClick={() => onSelect(null)}>
                  {option}
                </p>
              );
            } else {
